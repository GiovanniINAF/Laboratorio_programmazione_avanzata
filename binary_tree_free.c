...
/* define: metodo per memorizzare le definizioni
 in fase di compilazione tutte le volte che verrà incontrata
 la variabile MAXWORD, al suo posto verrà automaticamente scritto
 100
*/
#define MAXWORD 100
...
struct tnode {
    char *word;             // La parola
    int count;              // Quante volte è apparsa
...
// strdup non è nello standard C, quindi è buona norma scriverne una propria.
...
struct tnode *addtree(struct tnode *p, char *w) {
    ...
    // Qui è dove dobbiamo inserire la nuova parola.
    if (p == NULL) {
        p = talloc();
        p->word = my_strdup(w);
        p->count = 1;
    ...
    // La parola che stiamo inserendo (w) è uguale a quella del nodo corrente (p->word)
...
// freememory: libera la memoria di tutti i nodi dell'albero (visita post-order)
void freememory(struct tnode *p) {
    if (p != NULL) {
        // 1. Visita ricorsiva sul sotto-albero sinistro
        freememory(p->left);

        // 2. Visita ricorsiva sul sotto-albero destro
        freememory(p->right);

        // 3. Libera la memoria allocata per la stringa 'word'
        free(p->word);

        // 4. Libera la memoria allocata per il nodo stesso
        free(p);
    }
}
